  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      if (!data.questions.length) {
        setError("No questions generated. Try again or check AI settings.");
      }
    } catch (err) {
  ...
  return (
    <Stack spacing={2}>
      <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap" useFlexGap>
        <Typography variant="h6">Word quiz</Typography>
      ...
      {loading && !session && (
        <Stack direction="row" spacing={1} alignItems="center">
          ...
          <Typography variant="body2">Generating exercises...</Typography>
        </Stack>
      )}
