...
export function BookLessonPanel() {
  ...
  const bookLoading = bookQuery.isLoading && !book;
  const pageLoading = Boolean(pageId) && pageQuery.isLoading && !page;
  const actionLoading = completeMutation.isPending;
  const loadError =
  ...
  return (
    <FeaturePanel
      ...
      description="Grammar topics and quizzes from the lesson book."
      loading={bookLoading}
      error={loadError}
    ...
    >
      <Stack spacing={2}>
        ...
        {book && pageOptions.length === 0 && (
          <Alert severity="info">No structured lesson pages are available for {level} yet.</Alert>
        )}

        {pageLoading && pageOptions.length > 0 && (
          <Box sx={{ display: "flex", justifyContent: "center", py: 2 }}>
        ...
        {page && !pageQuery.isLoading && (
          <Stack spacing={2}>
            ...
            <Button
              ...
              disabled={completedIds.has(page.id) || actionLoading}
            >
